...
def get_id(id=None):
    ...
    try:
        ...
        time.sleep(0.5)
        response = requests.get(URL+method_name, params=request_params(method_name, id))
        user = response.json()['response'][0]['id']
    return str(user)
...
def alone_group(id=None, fp=None):
    # Функция использует get_id()  для получения id из аргументов функции, из командной строки или ввода пользователя
    # возвращает множество групп в которых состоит пользователь и не состоят его друзья
...
def get_groups_info(groups, fp=None):
    # Функция принимает ids группы (строка или число) или групп (список, кортеж, множество), возвращает id, имя и количество участников групп
